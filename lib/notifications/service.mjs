...
export const NOTIFICATION_TYPES = {
  ...
  THRESHOLD_ALERT: "threshold_alert", // M8: a table exceeded its size threshold
  SYSTEM: "system", // M7: generic system notice
};
export const NOTIFICATION_STATUSES = ["open", "assigned", "resolved", "dismissed"];
const OPEN_STATUSES = ["open", "assigned"];

const REF_PREFIX = { account_request: "AR", password_reset: "PR", threshold_alert: "TA", system: "NT" };
...
// ── generic system notification (M7) — for internal producers (e.g. the M8 storage
// monitor raising a threshold alert). NOT a public request: it mints a ref id and
// inserts an OPEN row directly (system actor; bypasses audit via prismaBase — the row
// IS the record). `dedupeKey` (optional) collapses repeat notices onto one open row
// (matched on label + an open status + the dedupe marker in `data.dedupeKey`) so a
// recurring monitor doesn't flood the queue. Returns { notification, created }.
export async function createNotification({ type = NOTIFICATION_TYPES.SYSTEM, label = null, title, body, entityType = null, entityId = null, data, dedupeKey } = {}) {
  if (!title) throw new CmsValidationError("A notification title is required.");
  if (dedupeKey) {
    const open = await prismaBase.notification.findFirst({
      where: { type, status: { in: OPEN_STATUSES }, data: { path: ["dedupeKey"], equals: dedupeKey } },
      include: WITH_PEOPLE,
      orderBy: { createdAt: "desc" },
    });
    if (open) return { notification: shapeNotification(open), created: false };
  }
  const referenceId = await mintReferenceId(type);
  const mergedData = { ...(data ?? {}), ...(dedupeKey ? { dedupeKey } : {}) };
  const row = await prismaBase.notification.create({
    data: {
      referenceId, type, label, status: "open", title,
...
export async function listNotifications({ status, type, label, assignedToUserId, take = 200 } = {}, actor = {}) {
...
// ── PURE keyset cursor (createdAt + id — notification.id is a uuid, not monotonic,
// so the createdAt-then-id composite gives a total, stable newest-first order). ──
...
// Keyset-paginated newest-first listing (M7). Returns { entries, nextCursor, hasMore }.
// Fetches take+1 to detect another page; the composite (createdAt, id) cursor is
// stable under inserts. Filters: status / type / label / assignedToUserId.
export async function listNotificationsPage({ status, type, label, assignedToUserId, cursor, take = 50 } = {}, actor = {}) {
  await assertActorPermission(actor, "notification.read");
  ...
  if (assignedToUserId) where.assignedToUserId = assignedToUserId;
  if (cursor) {
    const c = decodeNotificationCursor(cursor);
    where.OR = [
  ...
  return {
    entries: page.map(shapeNotification),
