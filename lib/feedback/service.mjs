  ...
  const { feedback } = await auditedMutation(
    ...
    async (tx) => ({
      feedback: await tx.feedback.update({
        ...
        data: {
          status,
          resolutionNote: note != null ? String(note).slice(0, 2000) : existing.resolutionNote,
          // Closing stamps resolvedBy/resolvedAt; RE-OPENING to a live status (triaged/
          // in_progress) CLEARS them so a now-live ticket can't show a stale resolution
          // (Session-11 review).
