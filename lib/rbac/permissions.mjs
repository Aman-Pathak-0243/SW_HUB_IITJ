...
export const PERMISSIONS = [
  ...
  // Member platform — M3 (Session 11): manage a club/society/chapter's memberships
  // (add / remove / bulk-import member emails). SCOPED to the unit's lineage via
  // requireScopedPermission (DL-066/075) — a coordinator scoped to their club can
  // manage only that club's members.
  { key: "membership.manage", module: "org", label: "Manage club/society memberships (add, remove, bulk-import)" },

  // Member platform — M5 (Session 11): run an event's OPERATIONAL subsystem —
  // organizer/collaborator tagging, rounds, scores, attendance, settings, registration
  // management, closure reports. Authorized via the `assertEventManage` seam (DL-086):
  // GLOBAL (staff/admin/dev) OR SCOPED to a tagged organizing club's lineage (a
  // coordinator runs their own event). Organizer tagging + closure REVIEW are central
  // (global). Member self-registration is login-only + assertCanParticipate (no perm).
  { key: "event.manage", module: "events", label: "Manage the event playground (organizers, rounds, scores, attendance, closure)" },
...
// coordinator runs a club: the editor set + manage that club's memberships (M3) +
// run that club's events (M5, SCOPED to the club lineage via assertEventManage).
const COORDINATOR_PERMS = [...CONTENT_EDITOR_PERMS, "membership.manage", "event.manage"];
const SECRETARY_PERMS = [
  ...CONTENT_EDITOR_PERMS,
  "membership.manage", "event.manage",
...
const STAFF_PERMS = [
  ...
  // Central event operations (M5) — staff hold event.manage unscoped, so they run
  // the playground centrally (organizer tagging, closure review) alongside admin.
  "event.manage",
];
