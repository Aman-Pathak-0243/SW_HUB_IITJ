...
// DB-backed RBAC inputs for a user, MEMOIZED per request (React.cache). Keyed only
// on userId (a primitive — so the memo actually hits; `scope` never touches the DB,
// it is applied by the pure resolver below). React.cache is request-scoped on the
// Next server, so NOTHING is cached across requests — a revoked role still takes
// effect on the next request (DL-013). This collapses the ~7 identical permission
// resolutions a single admin page triggers (layout + loadModuleContext + each gated
// read in lib/admin/reads.mjs) down to one set of DB round-trips. Never invoked by
// the CLI (a `system` actor short-circuits authorization before reaching here), so
// it never runs outside a React request scope.
// The RBAC INPUTS for a user (identity + normalized in-scope-agnostic assignments +
// overrides), un-memoized so a NON-request caller (a CLI/test/service outside a React
// request scope — e.g. lib/rbac/grants.mjs' scoped-grant discovery) can load them with
// an explicit client. `loadUserRbac` below wraps this in React.cache for the hot path.
// Behaviour is identical to the previous inline body (same selects, same early returns).
export async function loadUserRbacInputs(userId, client = prisma) {
  const user = await client.user.findUnique({
    where: { id: userId },
    select: { id: true, isDeveloper: true, status: true },
  });
  if (!user || user.status !== "active") return { user: user ?? null, normalized: [], normalizedOverrides: [] };
  // Developers/grants_all short-circuit in the resolver — no need to load the rest.
  if (user.isDeveloper) return { user, normalized: [], normalizedOverrides: [] };
  ...
  return { user, normalized, normalizedOverrides };
}

const loadUserRbac = cache((userId) => loadUserRbacInputs(userId));

// DB-backed resolution for a real user id. The DB read is memoized per request via
// loadUserRbac; `scope` is applied live by the pure resolver on every call, so the
// scope-specific decision is never cached.
export async function getEffectivePermissions(userId, scope = {}) {
  const { user, normalized, normalizedOverrides } = await loadUserRbac(userId);
