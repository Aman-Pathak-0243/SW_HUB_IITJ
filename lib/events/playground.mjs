...
import { isRegistrationOpen, registrationOutcome, rankEntries, isRoleAllowedToRegister } from "./forms.mjs";
import { shapeSettings } from "./settings.mjs";
import { getMyRegistration, getActiveRoleKeys } from "./registration.mjs";
...
export async function getPlaygroundEvent({ slug, id, userId, yearId, now = new Date(), client = prisma } = {}) {
  ...
  const item = await client.contentItem.findFirst({
    ...
    select: { id: true, slug: true, pinned: true, publishedAt: true, publishedRevisionId: true, academicYearId: true, orgUnitId: true },
  });
  if (!item) return null;
  // Resolve the org-unit LINEAGE key for the inline-edit capability scope (Session 15, DL-103);
  // null for a central (non-club-bound) event → capability then needs unscoped content.update.
  const orgUnit = item.orgUnitId ? await client.orgUnit.findUnique({ where: { id: item.orgUnitId }, select: { lineageKey: true } }) : null;

  const [rev, payload, rounds, tags, settingsRow, regGroups, scoreRows, myReg, myRoleKeys] = await Promise.all([
    ...
    userId ? getActiveRoleKeys(userId, client) : Promise.resolve(new Set()),
  ]);
  ...
  // Per-event allowed-registrant-roles (DL-097): empty = open to all. Eligibility is
  // computed for the signed-in member so the button can say "not eligible" up front
  // instead of letting the register POST 403; the server re-checks on submit.
  const allowedRoles = settings.allowedRegistrantRoles ?? [];
  const eligible = allowedRoles.length === 0 || (!!userId && isRoleAllowedToRegister(allowedRoles, myRoleKeys));
  ...
  return {
    id: item.id,
    slug: item.slug ?? null,
    // Server-only scope for the inline-edit capability check (DL-103) — not rendered.
    academicYearId: item.academicYearId,
    orgUnitLineageKey: orgUnit?.lineageKey ?? null,
    title: rev?.title ?? null,
    summary: rev?.summary ?? null,
    pinned: !!item.pinned,
    ...
    settings: {
      ...
      allowedRegistrantRoles: allowedRoles,
    },
    registration: {
      confirmed: counts.confirmed,
      waitlisted: counts.waitlisted,
      ...
      mine: myReg ?? null,
      // Item B — the window for the client countdown / auto-flip (DL-098).
      registrationOpensAt: settings.registrationOpensAt,
      registrationClosesAt: settings.registrationClosesAt,
      registrationClosed: settings.registrationClosed,
      // Item A — is the signed-in member's account type eligible to self-register (DL-097).
      eligible,
      allowedRegistrantRoles: allowedRoles,
    },
