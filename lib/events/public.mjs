...
// Flagship events are a dedicated curated content_type (flagship_event) — always public,
// so no audience gating. The payload carries description/imageMediaId/category (the image
// is mapped onto coverMediaId before hydrate so the shared batched resolver serves it).
function shapeFlagshipEvent(h) {
  const p = h.payload;
  return {
    id: h.item.id,
    slug: h.item.slug,
...
// Publicly-visible FLAGSHIP events for the current year (curated content_type=
// 'flagship_event'), pinned-first then most-recently-published. Reflects whatever an
// editor creates/publishes/pins through the CMS — no static list. The payload's
// imageMediaId is mapped onto coverMediaId so the shared batched hydrate serves the image.
export async function listPublicFlagshipEvents({ now = new Date(), currentYearId, client = prisma } = {}) {
  const recs = await listPublicContent({ contentType: "flagship_event", now, currentYearId }, { client });
  const mapped = recs.map((r) => ({ item: r.item, payload: { ...(r.payload ?? {}), coverMediaId: r.payload?.imageMediaId ?? r.payload?.coverMediaId ?? null } }));
  return (await hydrate(client, mapped)).map(shapeFlagshipEvent);
}
...
// A club's events are the UNION of (a) events BOUND to it via content_item.orgUnitId
// (its "home" events) and (b) events TAGGED to its DURABLE lineage as an organizer OR
// collaborator (event_organizer.orgUnitLineageKey) — so a collaboratively-organized
// event appears on EVERY tagged club's page, not just its home club (Session 14, DL-099).
// Pass `lineageKey` (the club's org_unit_lineage) to include the tagged set; without it
// the behaviour is exactly the prior orgUnitId-only listing. Deduped by content_item id.
export async function listClubEvents(orgUnitId, { yearId, lineageKey, now = new Date(), audiences = PUBLIC_AUDIENCES, client = prisma } = {}) {
  if (!orgUnitId) return [];
  const year = yearId ?? (await getCurrentYearId(client));
  if (!year) return [];

  // (a) home events bound via orgUnitId.
  const boundRecs = await loadPublicItems(client, publicItemWhere(year, { contentType: "event", orgUnitId }), { enforceWindow: false, now });

  // (b) events tagged to this club's lineage (organizer or collaborator). Restricted to
  // this year's published, non-archived events by the shared public where.
  let taggedRecs = [];
  if (lineageKey) {
    const tags = await client.eventOrganizer.findMany({ where: { orgUnitLineageKey: lineageKey }, select: { eventItemId: true } });
    const taggedIds = [...new Set(tags.map((t) => t.eventItemId).filter(Boolean))];
    if (taggedIds.length) {
      const where = { ...publicItemWhere(year, { contentType: "event" }), id: { in: taggedIds } };
      taggedRecs = await loadPublicItems(client, where, { enforceWindow: false, now });
    }
  }

  // Union + dedup by content_item id (a home event that is also tagged appears once).
  const byId = new Map();
  for (const r of [...boundRecs, ...taggedRecs]) byId.set(r.item.id, r);
  return filterByAudience((await hydrate(client, [...byId.values()])).map(shapeEvent), audiences);
}
