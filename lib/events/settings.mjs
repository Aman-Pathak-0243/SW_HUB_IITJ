...
import { normalizeCapacity, normalizeAllowedRoles } from "./forms.mjs";
...
export function shapeSettings(s, eventItemId) {
  if (!s) return { eventItemId, capacity: null, registrationOpensAt: null, registrationClosesAt: null, registrationClosed: false, allowedRegistrantRoles: [] };
  return {
    ...
    allowedRegistrantRoles: Array.isArray(s.allowedRegistrantRoles) ? s.allowedRegistrantRoles : [],
  };
}
...
export async function upsertEventSettings(eventItemId, patch = {}, actor = {}) {
  ...
  const allowedRoles = normalizeAllowedRoles(patch.allowedRegistrantRoles);
  if (allowedRoles !== undefined) data.allowedRegistrantRoles = allowedRoles;
