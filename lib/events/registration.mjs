...
import { isRegistrationOpen, registrationOutcome, normalizeRegistrationStatus, isRoleAllowedToRegister } from "./forms.mjs";
...
// Best-effort LIVE registration counts push (Session 16, DL-108) — the cheap first step
// on the SSE transport: every path that changes an event's registration state publishes
// the fresh PII-free counts to the `registration:{eventItemId}` channel. The live
// board (GET /api/live/registration/[eventItemId]) streams these. NEVER throws into the
// caller — a broadcast failure must not break a registration write.
async function publishRegistrationCounts(eventItemId) {
  if (!eventItemId) return;
  try {
    const counts = await getRegistrationCounts(eventItemId);
    await publish(channels.registration(eventItemId), { type: "counts", ...counts });
  } catch (e) {
    console.warn("[events] live registration publish failed:", e?.message ?? e);
  }
}

// The set of ACTIVE role keys a user holds (any scope) — used to gate self-registration
// against an event's allowed-registrant-roles allow-list (DL-097). Scope-independent on
// purpose: "staff" may register regardless of which unit they are staff of. Mirrors the
// active-assignment filter in lib/rbac/authorize.mjs#loadUserRbacInputs.
export async function getActiveRoleKeys(userId, client = prisma) {
  if (!userId) return new Set();
  const rows = await client.roleAssignment.findMany({
    where: { userId, revokedAt: null, role: { status: "active" } },
    select: { role: { select: { key: true } } },
  });
  return new Set(rows.map((r) => r.role?.key).filter(Boolean));
}
...
export async function registerForEvent(input = {}, member, actor = {}) {
  ...
  // Per-event allowed-registrant-roles gate (DL-097). Empty allow-list = open to all;
  // otherwise the member must hold one of the allowed role keys. Organizer manual-add
  // (addRegistration, assertEventManage) is the escape hatch and is NOT gated here.
  const allowedRoles = settings?.allowedRegistrantRoles ?? [];
  if (allowedRoles.length && !isRoleAllowedToRegister(allowedRoles, await getActiveRoleKeys(userId))) {
    throw new CmsValidationError("Your account type is not eligible to register for this event.", { status: 403, code: "ROLE_NOT_ELIGIBLE" });
  }
  ...
  await publishRegistrationCounts(item.id);
