...
import { inferPersonType, toTimeDate } from "./normalize.mjs";
...
export async function importOrgData(opts = {}, actor = SYSTEM_ACTOR) {
  ...
  // Uses the ONE shared inventory writer (lib/media/service.mjs); it dedups
  // non-base64 refs by natural key and — per DL-039 — keeps each base64 placeholder
  // a DISTINCT row (so the Session-7 migration reconciles them individually).
  ...
  async function ensurePerson({ name, positionKey, scope, profileUrl = null, phone = null, email = null, photo = null }) {
    ...
    const res = await upsertPerson({ fullName: name, personType, profileUrl, phone, email, photoMediaId }, actor, { scope });
  ...
  for (const council of plan.councils) {
    ...
    const councilLogoId = await ensureMedia(council.logo);
    await ensureProfile(councilUnit, "council_profile", council.name, { logoMediaId: councilLogoId });
    if (council.associateDean) {
      const ad = await ensurePerson({ name: council.associateDean.name, positionKey: "associate_dean", profileUrl: council.associateDean.profileUrl, photo: council.associateDean.photo, scope: scopeOf(councilUnit) });
      await ensureAppointment(councilUnit, "associate_dean", ad, { titleOverride: council.associateDean.titleOverride });
    }
    if (council.secretary) {
      const person = await ensurePerson({ name: council.secretary.name, positionKey: "council_secretary", email: council.secretary.email, photo: council.secretary.photo, scope: scopeOf(councilUnit) });
      await ensureAppointment(councilUnit, "council_secretary", person, { titleOverride: council.secretary.titleOverride });
    }

    let repOrder = 0;
    for (const r of council.representatives ?? []) {
      const person = await ensurePerson({ name: r.name, positionKey: r.positionKey, email: r.email, photo: r.photo, scope: scopeOf(councilUnit) });
      await ensureAppointment(councilUnit, r.positionKey, person, { titleOverride: r.titleOverride, sortOrder: repOrder++ });
    }
    ...
    for (const club of council.clubs) {
      ...
      if (club.pic) {
        const pic = await ensurePerson({ name: club.pic.name, positionKey: "pic", profileUrl: club.pic.profileUrl, email: club.pic.email, photo: club.pic.photo, scope: scopeOf(clubUnit) });
        await ensureAppointment(clubUnit, "pic", pic, { sortOrder: 0 });
      }
      let coordOrder = 0;
      for (const c of club.coordinators) {
        const person = await ensurePerson({ name: c.name, positionKey: c.positionKey, email: c.email, photo: c.photo, scope: scopeOf(clubUnit) });
        await ensureAppointment(clubUnit, c.positionKey, person, { titleOverride: c.titleOverride, sortOrder: coordOrder++ });
      }
      let assocOrder = 0;
      for (const a of club.associates ?? []) {
        const person = await ensurePerson({ name: a.name, positionKey: a.positionKey, email: a.email, photo: a.photo, scope: scopeOf(clubUnit) });
        await ensureAppointment(clubUnit, a.positionKey, person, { titleOverride: a.titleOverride, sortOrder: assocOrder++ });
      }
    }
  }
  ...
  for (const hostel of plan.hostels) {
    ...
    for (const r of hostel.roles) {
      const person = await ensurePerson({ name: r.name, positionKey: r.positionKey, email: r.email, phone: r.phone, photo: r.photo, profileUrl: r.profileUrl ?? null, scope: scopeOf(unit) });
      await ensureAppointment(unit, r.positionKey, person, { titleOverride: r.titleOverride ?? null, sortOrder: roleOrder++ });
    }
  }
  ...
  for (const mess of plan.messes) {
    ...
    if (mess.secretary) {
      const sec = await ensurePerson({ name: mess.secretary.name, positionKey: "mess_secretary", email: mess.secretary.email, photo: mess.secretary.photo, scope: scopeOf(unit) });
      await ensureAppointment(unit, "mess_secretary", sec, { titleOverride: mess.secretary.titleOverride });
    }
