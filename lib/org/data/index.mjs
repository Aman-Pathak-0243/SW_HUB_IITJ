...
function planClub(club) {
  const coordinators = (club.coordinators ?? []).map((c) => ({
    ...
    photo: c.photo ?? null,
    role: c.role ?? null,
    // e.g. "Coordinator (Boys)" / "Coordinator (Girls)" — the Sports Coordinators
    // notification splits several clubs by team, and the roster card shows this.
    titleOverride: c.titleOverride ?? null,
    positionKey: coordinatorPositionKey(c.role),
  }));
  // Staff attached to the club (2026-27 roster) — a support role alongside the
  // student coordinators, so it plans exactly like them under its own position key.
  const associates = (club.associates ?? []).map((a) => ({
    name: cleanName(a.name),
    email: a.email ?? null,
    photo: a.photo ?? null,
    titleOverride: a.titleOverride ?? null,
    positionKey: "club_associate",
  }));
  return {
    name: club.name,
    ...
    pic: club.pic ? { name: cleanName(club.pic.name), email: club.pic.email ?? null, profileUrl: club.pic.profileUrl ?? null, photo: club.pic.photo ?? null, positionKey: "pic" } : null,
    coordinators,
    associates,
  };
}
...
export function buildImportPlan() {
  const councils = COUNCILS.map((c) => ({
    ...
    secretary: c.secretary
      ? { name: cleanName(c.secretary.name), email: c.secretary.email ?? null, titleOverride: c.secretary.titleOverride ?? null, photo: c.secretary.photo ?? null, positionKey: "council_secretary" }
      : null,
    // Council-level student representatives (currently the Sports Council's PG rep).
    representatives: (c.representatives ?? []).map((r) => ({
      name: cleanName(r.name),
      email: r.email ?? null,
      photo: r.photo ?? null,
      titleOverride: r.titleOverride ?? null,
      positionKey: "pg_representative",
    })),
  ...
  const hostels = HOSTELS.map((h) => ({
    ...
    roles: [
      ...
      ...(h.roles ?? []).map((r) => ({ positionKey: r.position, name: cleanName(r.name), email: r.email ?? null, phone: r.phone ?? null, photo: r.photo ?? null, titleOverride: r.titleOverride ?? null, profileUrl: null })),
    ],
  }));
  ...
  const messes = MESSES.map((m) => ({
    ...
    image: m.image ?? null,
    mealTimings,
    // AY 2026-27: the mess secretary posts are assigned by AREA, so each mess names
    // its own holder (the singleton mess_secretary is per unit, not campus-wide).
    secretary: m.secretary
      ? { name: cleanName(m.secretary.name), email: m.secretary.email ?? null, titleOverride: m.secretary.titleOverride ?? null, photo: m.secretary.photo ?? null, positionKey: "mess_secretary" }
      : null,
  }));
