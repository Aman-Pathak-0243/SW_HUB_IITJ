...
async function resolveMedia(client, ids) {
  ...
  return Object.fromEntries(rows.map((m) => [m.id, { url: m.url, kind: m.kind }]));
}
...
export async function getPublicOrgUnit(slug, { yearId } = {}, { client = prisma } = {}) {
  ...
  const [profile, roster] = await Promise.all([loadProfile(client, unit), loadRoster(client, unit)]);
  ...
  return { year, unit: shapeUnit(unit), profile, roster, children, media };
}
