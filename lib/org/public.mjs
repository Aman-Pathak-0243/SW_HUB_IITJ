...
export async function getClubPageView(slug, { yearId, now = new Date() } = {}, { client = prisma } = {}) {
  ...
  if (!expanded) {
    return { ...base, expanded: false, events: { past: [], upcoming: [] }, announcements: { past: [], current: [], upcoming: [] }, docs: [], memberCount: 0 };
  }
  const year = base.year;
  const [clubEvents, clubAnnouncements, docs, memberCount] = await Promise.all([
    listClubEvents(base.unit.id, { yearId: year, now, client }),
