...
// The per-MEMBER Wall-of-Fame slice (M6, DL-090) — the achievements CREDITED to one
// member (achievement_credit.userId, the durable account id), published + current-year,
// most-recent first. Fills the member PROFILE's achievements section (self + admin).
// Same batched hydrate + PII-minimized shape as the club slice (co-credited members
// appear by display NAME only). `userId` is the durable app_user id.
export async function listMemberAchievements(userId, { yearId, client = prisma } = {}) {
  if (!userId) return [];
  const year = yearId ?? (await getCurrentYearId(client));
  if (!year) return [];
  ...
  // Only the member's credited achievements that are actually publicly visible this year.
  const items = await client.contentItem.findMany({
