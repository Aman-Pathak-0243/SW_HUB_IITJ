...
import { CmsValidationError, CmsNotFoundError, withMappedDbErrors } from "./errors.mjs";

const ENTITY = "content_item";

// Interactive-transaction options. A CMS operation runs several sequential
...
// generous SAFETY ceilings — when Neon is warm the work finishes in well under a
// second; they never trip in normal serverless operation (mirrors the values the
// Session-2 DB smoke test uses).
const TX_OPTS = { timeout: 30000, maxWait: 20000 };
...
// Run a mutation transaction with auto-audit suppressed, then write one semantic
// audit row after commit (best-effort — committed data is never rolled back by a
// failed audit write; see DL-028).
async function auditedMutation(actor, txFn, auditEntryFor) {
