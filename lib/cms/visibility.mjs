...
// The current academic year row (exactly one, by the is_current partial unique).
...
export async function listPublicContent(
  ...
  const items = await client.contentItem.findMany({
    where: publicItemWhere(yearId, { contentType, orgUnitId }),
  ...
  for (const item of items) {
    ...
    if (WINDOWED_TYPES.has(item.contentType) && !isWithinPublishWindow(payload, now)) continue;
...
export async function getPublicItemBySlug(contentType, slug, { now = new Date(), client = prisma } = {}) {
  ...
  const item = await client.contentItem.findFirst({
    where: { ...publicItemWhere(yearId, { contentType }), slug },
  });
  if (!item) return null;
  const handler = getContentTypeHandler(item.contentType);
  const payload = handler ? await handler.readPayload(client, item.publishedRevisionId) : null;
  if (WINDOWED_TYPES.has(item.contentType) && !isWithinPublishWindow(payload, now)) return null;
  return { item, payload };
}
