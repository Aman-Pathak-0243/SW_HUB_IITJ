...
// (or an explicit `currentYearId`), or null if it does not exist / is not public.
export async function getPublicItemBySlug(contentType, slug, { now = new Date(), currentYearId, client = prisma } = {}) {
  const yearId = currentYearId ?? (await resolveCurrentYear(client))?.id;
