...
//  - Event playground tables (M5): audited SEMANTICALLY by lib/events/* — organizer
//    tagging / rounds / scores / attendance / settings / closure each write ONE
//    summary row per business action (cross-stakeholder ops); MEMBER self-registration
//    is the durable row itself and is NOT audited (a routine self-service action, like
//    a page visit) — organizer-initiated registration changes ARE audited. The
//    event_payload itself is a CMS payload (written in the content service's suppressed
//    transaction like every *_payload) — NOT skipped here.
//  - Live-quiz tables (Session 16): audited SEMANTICALLY by lib/quiz/* — organizer
//    question authoring + session lifecycle (create/start/next/reveal/end) each write
//    ONE summary row per business action; MEMBER lobby joins (quiz_participant) and
//    answers (quiz_answer) are the durable rows themselves and are NOT audited (routine
//    high-volume self-service, like a registration / page visit).
const AUTO_AUDIT_SKIP = new Set([
  ...
  "AchievementCredit",
  "EventEntity", "EventOrganizer", "EventSettings", "EventRound", "EventRegistration",
  "EventScore", "EventAttendance", "EventClosureReport",
  "QuizQuestion", "QuizSession", "QuizParticipant", "QuizAnswer",
]);
