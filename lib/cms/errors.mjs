...
const GUARD_MATCHERS = [
  ...
  // Event playground (M5) guard signatures.
  {
    test: (t) => t.includes("event_registration_capacity_guard"),
    make: () =>
      new CmsError("This event is full — no confirmed spots remain (try the waitlist).", {
        status: 409,
        code: "EVENT_FULL",
      }),
  },
  {
    test: (t) => t.includes("event_registration_active_uq"),
    make: () => new CmsError("You are already registered for this event.", { status: 409, code: "ALREADY_REGISTERED" }),
  },
  {
    test: (t) => t.includes("event_organizer_one_target_chk"),
    make: () =>
      new CmsError("An organizer tag must target exactly one of a club, a custom entity, or a member.", {
        status: 422,
        code: "ORGANIZER_ONE_TARGET",
      }),
  },
  {
    test: (t) => t.includes("event_round_item_no_uq"),
    make: () => new CmsError("A round with that number already exists for this event.", { status: 409, code: "ROUND_NO_TAKEN" }),
  },
  {
    test: (t) => t.includes("event_entity_name_uq"),
    make: () => new CmsError("A custom entity with that name already exists.", { status: 409, code: "ENTITY_NAME_TAKEN" }),
  },
