...
// Every publicly-visible resource for a year, grouped by unit WITH the unit's display
// info — powers the public /resources index page. Batched (items + hydrate + the unit
// name comes from the same query). Returns [{ unit:{name,slug,typeKey}, resources:[…] }]
// sorted by unit name.
export async function listAllPublicResources({ yearId, client = prisma } = {}) {
  const year = yearId ?? (await getCurrentYearId(client));
  if (!year) return [];
  const items = await client.contentItem.findMany({
    where: {
    ...
    select: { ...publishedResourceSelect, orgUnit: { select: { name: true, slug: true, orgUnitType: { select: { key: true } } } } },
    orderBy: { createdAt: "asc" },
  });
  if (!items.length) return [];
  ...
  items.forEach((item, i) => {
    if (!byUnit.has(item.orgUnitId)) {
      byUnit.set(item.orgUnitId, {
        unit: { name: item.orgUnit?.name ?? null, slug: item.orgUnit?.slug ?? null, typeKey: item.orgUnit?.orgUnitType?.key ?? null },
        resources: [],
      });
    }
    byUnit.get(item.orgUnitId).resources.push(shaped[i]);
  });
  return [...byUnit.values()].sort((a, b) => (a.unit.name ?? "").localeCompare(b.unit.name ?? ""));
}
