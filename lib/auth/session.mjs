...
// THE ACCOUNT-CREDENTIAL boundary (M0, consolidation review B1) — for a self-service
// operation on one's OWN account (the forced first-login change + the normal "change my
// password" path). Admits any account that CAN LOG IN (active OR inactive), rejects
// `revoked`, and — unlike requireMember() — does NOT consult the member-view
// `allowNormalView` toggle: changing your own password is a credential operation, not a
// member-view surface. Using the active-only requireUser() here permanently locked out an
// inactive account that the must-change middleware force-redirects to the change form
// (the POST 403'd even though changeOwnPassword has no status gate). Status is read LIVE.
export async function requireLoggedInAccount() {
  const session = await getServerAuthSession();
  if (!session?.user?.id) {
    const err = new Error("Unauthorized");
    err.status = 401;
