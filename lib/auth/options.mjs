...
// non-active accounts, and wrong passwords. A must-change account CAN sign in
// (it is still active) — the forced change is enforced after authentication via
// `mustChangePassword` on the session + the middleware.
...
export const authOptions = {
  ...
  callbacks: {
    // Block suspended/disabled accounts at the authentication boundary (BOTH OAuth
    // and credentials — credentials.authorize already rejects non-active, this also
    // covers a previously-linked Google account). Additionally, when the member
    // platform PLUGIN is enabled, REJECT Google sign-ins entirely (email+password
    // only, DL-058) — checked live from the DB flag (the signIn callback runs on the
    // Node runtime, so a Prisma read is available here).
