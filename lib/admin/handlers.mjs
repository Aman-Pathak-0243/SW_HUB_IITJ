...
import { createDraft, editDraft, publish, unpublish, archive, restore, editAndPublish } from "../cms/content.mjs";
...
import { createQuestion, editQuestion, deleteQuestion, reorderQuestions } from "../quiz/questions.mjs";
import { createSession, nextQuestion, revealQuestion, endSession } from "../quiz/sessions.mjs";
...
export const ADMIN_ACTIONS = {
  ...
  // Session 15 (DL-103): inline "save & make live" from a public page — edit the draft then
  // publish, each re-authorized at the item's (year, org-lineage) scope inside the service.
  "content.editAndPublish": { scoped: true, run: (a, actor) => editAndPublish(a.itemId, a.patch ?? {}, actor) },
  ...
  // ── Live quizzes (Session 16) — SCOPED: every quiz service authorizes via the SAME
  // assertEventManage seam (DL-104), so a coordinator runs their own event's quiz and
  // staff/admin/dev run any. Question authoring + session lifecycle only; member play
  // (join / answer) is the login-only /api/live/* routes, not this registry. ──
  "quiz.question.create": { scoped: true, run: (a, actor) => createQuestion(a.eventItemId, a.input ?? {}, actor) },
  "quiz.question.edit": { scoped: true, run: (a, actor) => editQuestion(a.questionId, a.patch ?? {}, actor) },
  "quiz.question.delete": { scoped: true, run: (a, actor) => deleteQuestion(a.questionId, actor) },
  "quiz.question.reorder": { scoped: true, run: (a, actor) => reorderQuestions(a.eventItemId, a.orderedIds ?? [], actor) },
  "quiz.session.create": { scoped: true, run: (a, actor) => createSession(a.eventItemId, actor) },
  "quiz.session.next": { scoped: true, run: (a, actor) => nextQuestion(a.sessionId, actor) },
  "quiz.session.reveal": { scoped: true, run: (a, actor) => revealQuestion(a.sessionId, actor) },
  "quiz.session.end": { scoped: true, run: (a, actor) => endSession(a.sessionId, actor) },
