...
// MEMOIZED per request (React.cache, no args) so the layout's call and every page's
// loadModuleContext() call share ONE resolution instead of each re-running the
// session lookup + permission read. Request-scoped only — never stale across requests.
export const loadAdminContext = cache(async () => {
