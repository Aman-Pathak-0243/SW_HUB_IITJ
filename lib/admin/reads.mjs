...
export async function loadPeople({ search, take = 500 } = {}, actor = {}) {
  await assertActorPermission(actor, "org_unit.read");
  const rows = await prisma.person.findMany({
    ...
    select: {
      ...
      // Their published, non-archived roles (position + unit) so the directory shows
      // WHO each person is (AD / secretary / PIC / warden / coordinator …).
      appointments: {
        where: { archivedAt: null, status: "published" },
        select: { titleOverride: true, position: { select: { name: true } }, orgUnit: { select: { name: true } } },
        take: 12,
      },
    },
  });
  return rows.map((p) => ({
    ...
    roles: (p.appointments ?? []).map((a) => ({ title: a.titleOverride ?? a.position?.name ?? null, unit: a.orgUnit?.name ?? null })),
  }));
}
