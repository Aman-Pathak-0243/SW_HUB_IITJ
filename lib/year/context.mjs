  ...
  return Promise.all(
    years.map(async (y) => {
      const [orgUnits, contentItems, appointments] = await Promise.all([
        client.orgUnit.count({ where: { academicYearId: y.id, archivedAt: null } }),
        client.contentItem.count({ where: { academicYearId: y.id, archivedAt: null } }),
        client.appointment.count({ where: { academicYearId: y.id, archivedAt: null } }),
      ]);
      return { ...y, counts: { orgUnits, contentItems, appointments } };
    })
  );
}
