...
// Cap on a single export so a filter-less "download everything" can't pull the whole
// (unbounded) audit table into memory. The dev tab paginates for browsing; export is
// the most-recent `EXPORT_MAX` matching the filter (narrow the filter for older rows).
const EXPORT_MAX = 5000;

// ── PURE: CSV serialization (M8) ──────────────────────────────────────────────
// Cell serialization is the shared lib/csv/cell.mjs (RFC-4180 quoting + formula-injection
// guard, consolidation review B6). Re-exported so existing importers keep the symbol.
// Bulk export carries the same data-minimized columns as the LIST view (who/what/when +
// summary) — NOT ip / user-agent / before-after (DL-047).
export { csvCell };
export const AUDIT_EXPORT_COLUMNS = ["id", "createdAt", "action", "entityType", "entityId", "actorEmail", "summary"];
export function auditEntriesToCsv(entries = [], columns = AUDIT_EXPORT_COLUMNS) {
  const header = columns.join(",");
  const rows = entries.map((e) =>
    columns
      .map((c) => csvCell(c === "actorEmail" ? e.actor?.email ?? null : e[c]))
      .join(",")
  );
...
// Export the Action Log / Change History (M8, DL-068) as JSON or CSV. Gated on
// audit.read like every other reader; returns { format, filename, contentType,
// content, count }. Fetches up to EXPORT_MAX newest rows matching the (cursor-less)
// filter — narrow the filter (entity/actor/action/date) to reach older rows.
export async function exportAuditLog(rawFilters = {}, actor = {}, { format = "json", max = EXPORT_MAX, client = prisma } = {}) {
  await authorizeConsole(actor, READ_PERMS);
  const fmt = format === "csv" ? "csv" : "json";
  const filters = normalizeAuditFilters({ ...rawFilters, cursor: undefined });
  const cap = Math.min(Math.max(Number(max) || EXPORT_MAX, 1), EXPORT_MAX);
  ...
  const entries = rows.map((r) => shapeAuditEntry(r));
  const stamp = "audit-log";
  if (fmt === "csv") {
    return { format: "csv", filename: `${stamp}.csv`, contentType: "text/csv", content: auditEntriesToCsv(entries), count: entries.length };
  }
  return {
    format: "json",
    filename: `${stamp}.json`,
