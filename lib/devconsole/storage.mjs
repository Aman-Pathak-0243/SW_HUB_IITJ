...
// Minimal CSV via the shared cell serializer (RFC-4180 quoting + formula-injection guard —
// a full-table dump is exactly where an attacker-controlled string cell could smuggle a
// spreadsheet formula, consolidation review B6).
...
export async function exportTable(tableName, actor = {}, { format = "json", max = EXPORT_MAX_ROWS } = {}) {
  ...
  // FAIL-CLOSED (consolidation review B9): the audit row is written BEFORE the dump is
  // returned, and a failure ABORTS the export — a sensitive full-table dump must never be
  // handed back untracked. (The row goes through recordAudit on prismaBase, independent of
  // whether the actor holds backup.create / dev.console; only a hard DB error can fail it.)
  try {
    ...
    console.error(`[storage] export audit for ${name} failed — aborting (no untracked dump):`, e?.message ?? e);
    throw e;
  }
