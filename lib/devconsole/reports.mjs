...
export async function getDevConsoleReports(actor = {}, { client = prisma, tokenUsagePath } = {}) {
  ...
  let tokenUsage;
  try {
    // turbopackIgnore: see KNOWN_ISSUES #32 — keep this runtime fs read out of the
    // NFT trace; the file is bundled explicitly via outputFileTracingIncludes.
    const p = tokenUsagePath ?? path.join(/* turbopackIgnore: true */ process.cwd(), "docs", "Token_Usage.md");
