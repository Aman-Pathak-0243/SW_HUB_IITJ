...
| [WEBSITE_V1_VS_V2.md](WEBSITE_V1_VS_V2.md) | **Before → after** — what the original "normal website" (V1) had, what was wrong with it, and what each area became in the V2 platform, with a full feature-comparison table. |
| [ACADEMIC_YEAR_ROLLOVER.md](ACADEMIC_YEAR_ROLLOVER.md) | The **year-change runbook** — how to roll the platform into a new academic year (create year → Transition Wizard → set current → lock) and **how to ingest new data** (the importers), what carries over automatically vs. needs action, verification + a checklist. |
